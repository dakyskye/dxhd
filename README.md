...
rewrite process of dxhd.  The reason why
...
Download the `dxhd` executable file from the latest release, from
[releases page](https://github.com/dakyskye/dxhd/releases)

and copy `dxhd` executable file to somewhere in your `$PATH`

**Note:** `go get`ting dxhd is possible, but not recommended.  Read more
...
* key release events (`super + @key` where `key` is a non-modifier key, and `@`
  is a specifier)
* mouse button press events (`mouseN` where `n` is button number)
* mouse button release events (`@mouseN` where `n` is button number, and `@` is
  a specifier)
* variants (`{a,b,c}`)
...
* support for any shell scripting language (sh, bash, ksh, zsh, python, perl
  etc.) given as a shebang
* support for global variable declarations in a config
* support for scripting, as much as a user wishes!
* support for running as many dxhd instances simultaneously as you want, to
  logically separate your keybindings.
...
A dxhd config file should contain a shebang (defaults to `/bin/sh`) on top of a
file, which will be the shell used for executing commands.
...
uses `/usr/sh` shell to achieve it, as Go does not allow forking a process
...
* dxhd uses shebang to determine which shell to use (so you don't have to set an
...
They haven't benchmarked yet, so I don't know.  However, been using dxhd since
...
Whilst `go get`ting dxhd should work fine, it's not recommended, because we
can't know what version of dxhd you use in case you want to open a bug report or
so.  +It's not like dxhd has any bug issue is not opened for already, since the
developer of dxhd himself uses dxhd daily, but still.
